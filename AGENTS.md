...
- **Roll dates differ by product — derive them from volume, never assume.** COMEX metals leave the delivery month around first notice (end of the month *before* delivery), roughly three weeks earlier than the equity-index roll: copper and silver rolled H26 → K26 on 2026-02-24/25, and the old `hg`/`si` files sat on the *expiring* contract for all of early March (1–2 contracts a minute against 17–19). `build_metals_roll_calendar.py` derives the boundaries from daily volume crossover; `download_hg_si_1min.py`, `download_es_1min.py` and `download_gc_1min.py` then fetch one request per contract. The corrected bars are `{hg,si}_1min_2025_2026_frontmonth.csv` — prefer those over the same-named files without the suffix.
