...
def reference_summary(values, groups):
    ...
    answer = {"n": n, "g": g, "mean": float(mean) if mean is not None else None}
    if n < 2:
    ...
    radius = t.ppf(0.975, g - 1) * se
    return dict(answer, variance=float(variance), se=se,
...
def assert_number(actual, expected, label):
    if expected is None:
        ...
        assert isclose(actual, expected, rel_tol=1e-10, abs_tol=1e-9), (label, actual, expected)
...
if __name__ == "__main__":
    ...
    cases = json.loads(Path('/tmp/mnq-wick-phase-a-independent-oracles.json').read_text())
    for case in cases:
    ...
    print('Independent rational audit helpers verified on four cluster oracles and degenerate cases.')
