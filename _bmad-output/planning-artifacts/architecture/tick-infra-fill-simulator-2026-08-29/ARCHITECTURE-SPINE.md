...
- **Rule:** `sim.py` is the **sole driver** of this seam. Immediately after each `apply_event`, `sim` calls `queue_model.observe_book_event(tracker, record, resting_before)` (`resting_before` = the venue order at `record.order_id` looked up *before* `apply_event`; `None` for a `T`). It updates per-order counters by these enumerated rules only: a `T` (trade) at or through a working passive order's limit, with `ts_event` **strictly after** that order's arrival (AD-20), **adds its size to that order's `cum_trade_vol_since_arrival`** — it does **not** touch `queue_ahead`; a `C` (full cancel), a same-price size-reducing `M`, or a **price-changing `M`** of a **resting order ahead of us** (earlier `add_ts_ns`; ties → our order last) decrements `queue_ahead` — by the removed size for a same-price size-down, by the **full** resting size for a `C` or a price-changing `M` (which leaves our level entirely; `apply_event` does remove-old + add-new). Nothing else moves either counter. `fills.decide` is then a pure function of the resulting tracker state. *(Amended 2026-08-29 — Alex decision: the original "a `T` … decrements `queue_ahead`" clause was wrong. Trades feed `cum_trade_vol_since_arrival` only. The fill formula (AD-22) is cumulative — `cumulative = clamp(cum_trade_vol_since_arrival − queue_ahead, 0, size)` — and `fills.decide` emits the this-tick delta `cumulative − filled_qty` (AD-19). The `C` / price-changing-`M` full-size rule and the `sequence` tie-break being moot were confirmed at review-1. See `_bmad-output/implementation-artifacts/spec-ticksim-fills.md`.)*
