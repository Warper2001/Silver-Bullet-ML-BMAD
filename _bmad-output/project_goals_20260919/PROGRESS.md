...
Engineering is implemented in `research/project_goals/`; evidence remains provisional or insufficient where listed. No strategy parameter, frozen comparison code, threshold, holdout access or account capital changed. Reviewed changes were merged into main; MIM was restarted after a fresh broker flat/open-order check, and the research timer was installed and enabled. Existing journals were continued without reset. Full-session evidence commissioning remains pending.
...
| 4. Trustworthy execution | Independent persisted price provenance; fail-closed lookup; singleton completion+1s poll wrapper, health and lossless snapshot storage | Frozen checks pass; collector zero eligible sessions; full-session commissioning pending | Deployed September 19; inspect September 21 complete coverage; eligibility also requires prior-session context (possibly September 22 earliest); retain June 10, 2027 deadline |
...
Calibration/code were committed in b929884; the deliberately blocked registration template followed in 1717b33. This is not an approved experiment. Merge 9c2a4eb preserves the concurrently advancing main commits and pre-existing AGENTS.md edits. No remote push was performed.
...
## Rollout receipt — September 19, 2026 UTC

Merge: `9c2a4eb84263fe559a94c07e68b2aa7f3768ce59`. All **202 focused tests** passed on merged main (131 MIM + 71 research); the earlier 112 frozen-comparison tests also passed. The 14 frozen source files remain identical. Source/test Flake8 F checks and systemd validation passed. Generated CSVs retain standard CRLF endings; whitespace validation permits CR at EOL so input/output hashes remain intact.

[MIM restart receipt](verification/mim-rollout.json), [fresh broker flat check](rollout-flat-check.json), [startup excerpt](verification/mim-startup.log): restart at 19:34:13 UTC, account 26556101 flat with zero open orders. Runtime rederived MNQZ26 previous close 29,916.25 from September 18 contract-matched bars, restored 390 full-depth labels and 14 sigma days unchanged, and reconciled FLAT without account action. No other trader was started/stopped/restarted. Idle persisted state remains unchanged until the next normal save writes the new provenance field; further restarts safely rederive it.

`project-goals-shadow.timer` is enabled and running. [Three completed poll receipts](verification/collector-polls.json) show 20.72–23.72 seconds processing, followed by roughly 1.18 seconds before the next start. All frozen preflights pass. Both new snapshots per poll were verified and archived; initial compressed storage was 3.46 MB, then roughly 4.15 KB per idle poll through chunk reuse. Active-session growth still needs measurement. [Eligibility health](verification/collector-health.json): zero eligible sessions; six historical sessions remain excluded. No observations were reset and no interim efficacy statistics were inspected.

A live hung child is reported without being killed; overlap remains blocked. Known late completion recovers automatically. Orphaned/unhandled child receipts require operator review and explicit permission before any process kill. The research wrapper cannot guarantee recovery from arbitrary hangs without that authorization.

Next operational review: September 21 complete coverage, then September 22 eligibility if valid prior-session context is available. The frozen 120-session endpoint and June 10, 2027 deadline remain unchanged. Written provider agreements/reset history, actual operating invoices, attributable SIM fills, and paired corrected GAP calibration/fresh registered observations remain unresolved dependencies. No allocation, revenue, payout, capital-transfer or GAP edge conclusion is authorized by these artifacts.
