"""
PL (platinum) slippage measurement — frozen analysis (precommit 2026-06-26).

Evaluates data/quotes/pl_quote_capture.csv against the PRE-COMMITTED rule in
_bmad-output/precommit_pl_slippage_measurement_2026-06.md:

  Binding symbol:      the platinum contract (PLN26 / PLV26) with the GREATER
                       count of valid RTH samples across qualifying sessions.
  ...
  All-in cost:         c = pooled_median_spread + COMMISSION ($4.00/RT)
  PASS iff:            c <= $41.71/RT (netPF>=1.10)  AND  every qualifying
                       session's all-in median <= $62.02/RT (pure-breakeven guard)

PASS authorizes only WRITING a Gate-1 prereg for PL on a sealed holdout.
Combine-fit (full-contract notional vs 50K account) is a separate gate.
...
def main():
    ...
    tabs = {s: session_table(df, s) for s in SYMS}
    counts = {s: len(t) for s, t in tabs.items()}
    binding = max(counts, key=counts.get)
    other = [s for s in SYMS if s != binding][0]
    print(f"Valid RTH sample counts: {counts}  → binding symbol = {binding}")
    sil = tabs[binding]
    ...
    for s, r in per.iterrows():
        ...
        print(f"  {s}  n={int(r['n']):>5}  spread_median=${r['median_usd']:.2f} "
              f"({r['median_usd']/(TICK*PV):.1f}t)  all-in=${allin:.2f}  [{q}]")
    ...
    if g_cost and g_sess:
        print("\n🟢 PASS — measured platinum cost clears the frozen bar. Authorized "
              "next step: WRITE a Gate-1 prereg for PL on a sealed holdout using the "
    ...
    if len(tabs[other]):
        om = tabs[other]["spread_usd"].median()
        print(f"\n[context] {other} median RTH spread: ${om:.2f} ({om/(TICK*PV):.1f} ticks)")
