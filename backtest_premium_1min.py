#!/usr/bin/env python3
"""
Premium Strategy Backtest - 1-Minute Data (Correct Timeframe)

Uses REAL 1-minute MNQ data from /root/mnq_historical.json
Period: Dec 2023 - Mar 2026 (28 months)
Total bars: 795,296

This is the CORRECT timeframe for the live trading system.
"""
...
print("🚀 SILVER BULLET PREMIUM - 1-MINUTE BACKTEST")
print("=" * 70)
print("✅ Using CORRECT timeframe (1-minute)")
print("📂 Data: /root/mnq_historical.json")
print("📅 Period: Dec 2023 - Mar 2026")
print("=" * 70)
...
for bar_data in tqdm(raw_data, desc="Processing bars"):
    try:
        ...
        # Filter to 2025 data (same as previous backtests)
...
print(f"   Period: {bars_list[0]['timestamp']} to {bars_list[-1]['timestamp']}")

# Run premium strategy detection
print(f"\n🎯 Step 3: Running Premium Pattern Detection (1-MINUTE)...")

# Use FINAL optimized parameters from before
config = {
    'min_fvg_gap': 75.0,
...
print(f"\n🔧 PREMIUM PARAMETERS:")
print(f"   Timeframe: 1-MINUTE (CORRECT)")
...
print(f"\n   ✅ Detected {len(detected_setups)} setups on 1-minute data")
...
print(f"\n{'=' * 70}")
print("📊 COMPARISON: 5-MIN vs 1-MINUTE")
print("=" * 70)
print(f"{'Metric':<25s} {'5-Minute':>15s} {'1-Minute':>15s} {'Change':>15s}")
print("-" * 70)
print(f"{'Total Trades':<25s} {'15':>15s} {f'{len(trades_df)}':>15s} {f'{len(trades_df) - 15:+d}':>15s}")
print(f"{'Win Rate':<25s} {'60.00%':>15s} {f'{win_rate:.2f}%':>15s} {f'{win_rate - 60:+.2f}%':>15s}")
print(f"{'Total Return':<25s} {'$20,128':>15s} {f'${total_return:,.0f}':>15s} {f'${(total_return - 20128)/1000:+.1f}K':>15s}")
print(f"{'Profit Factor':<25s} {'2.53':>15s} {f'{profit_factor:.2f}':>15s} {f'{profit_factor - 2.53:+.2f}':>15s}")

# ML Training readiness
print(f"\n🤖 ML TRAINING READINESS:")
print(f"   Sample Size: {len(trades_df)} trades")
if len(trades_df) >= 1000:
    print(f"   Status: ✅ EXCELLENT (optimal for ML training)")
elif len(trades_df) >= 500:
    print(f"   Status: ✅ GOOD (sufficient for ML training)")
elif len(trades_df) >= 200:
    print(f"   Status: ⚠️  ACCEPTABLE (minimum for basic ML)")
else:
    print(f"   Status: ❌ INSUFFICIENT (need more data)")

# Save results
output_path = Path("data/reports/premium_backtest_1min_2025.csv")
...
print(f"\n" + "=" * 70)
print("✅ 1-MINUTE BACKTEST COMPLETE")
print("=" * 70)
print("This is the CORRECT timeframe for live trading")
print("All previous 5-minute backtests are INVALID")
print("=" * 70)
