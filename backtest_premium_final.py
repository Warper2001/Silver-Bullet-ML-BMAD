#!/usr/bin/env python3
"""
Final Optimization for Premium Strategy - Maximum Sample Size

Applies all diagnostic insights with maximum sample size:
- Inverted quality score (0-85 range - maximum trades)
- Wider stops (1.5x multiplier)
- No max FVG size constraint
- Goal: Generate 1000+ trades for ML training data generation
"""
...
print("🚀 SILVER BULLET PREMIUM - FINAL OPTIMIZATION")
print("=" * 70)
print("Method: Maximum sample size with diagnostic insights")
...
# Run final optimized strategy detection
print(f"\n🎯 Step 2: Running Premium Pattern Detection (FINAL OPTIMIZATION)...")

# FINAL Configuration - Maximum Sample Size
config = {
    'min_fvg_gap': 75.0,           # Original minimum
    'min_volume_ratio': 2.0,
    'max_bar_distance': 7,
    'min_quality_score': 0.0,      # NO MINIMUM (was 60)
    'max_quality_score': 85.0,     # Maximum to avoid worst quality
...
print(f"   FVG Size: ${config['min_fvg_gap']}+ (no maximum)")
print(f"   Quality Score: 0-{config['max_quality_score']} (MAXIMUM RANGE!)")
print(f"   Stop Multiplier: {config['stop_multiplier']}x (wider stops)")
print(f"   Expected Trades: 1000-2000 (ML training ready)")
...
for i in range(lookback, len(bars_list) - lookback):
    ...
    # Detect FVG with FINAL filter (maximum sample size)
    if i >= 2:
...
print(f"\n   Detected {len(detected_setups)} final optimized setups")
...
print("=" * 70)
print(f"{'Metric':<20s} {'Original':>15s} {'Final':>15s} {'Change':>15s}")
print("-" * 70)
...
# Success assessment
print(f"\n{'=' * 70}")
if len(trades_df) >= 500 and win_rate > 25 and profit_factor > 1.0:
    print("✅ OPTIMIZATION SUCCESSFUL - READY FOR ML TRAINING!")
    print("   Sufficient sample size for ML model training")
    print("   Strategy shows positive expectancy")
    print("   Can proceed to generate ML training data")
elif len(trades_df) >= 200 and win_rate > 20:
    print("⚠️  OPTIMIZATION ACCEPTABLE - ML FEASIBLE")
    print("   Sample size adequate for basic ML training")
    print("   Strategy shows improvement potential")
else:
    print("❌ OPTIMIZATION INSUFFICIENT")
    print("   Further strategy refinement needed")

print("=" * 70)

# Save results
output_path = Path("data/reports/premium_backtest_final.csv")
output_path.parent.mkdir(parents=True, exist_ok=True)
trades_df.to_csv(output_path, index=False)

summary_path = Path("data/reports/premium_backtest_final_summary.json")
with open(summary_path, 'w') as f:
    json.dump({
        ...
        'annual_projection': {
            ...
            'daily_return': float(annual_return/trading_days)
        },
        'optimization_params': {
            'fvg_size': f"${config['min_fvg_gap']}+ (no max)",
            'quality_range': f"0-{config['max_quality_score']}",
...
print("=" * 70)
print("Applied all diagnostic insights:")
print("1. Wider stops (1.5x) to reduce premature stop hits")
print("2. Inverted quality filtering (0-85) to focus on winning setups")
print("3. No maximum FVG size constraint (maximum sample size)")
print("\nNext Step: Generate ML training data with these optimized parameters")
print("=" * 70)
