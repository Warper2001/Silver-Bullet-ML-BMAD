#!/usr/bin/env python3
"""
Premium Strategy Backtest - FINAL CORRECT IMPLEMENTATION

All three components + HARD KILLZONE FILTER:
1. MSS (Market Structure Shift)
2. FVG (Fair Value Gap)
3. LIQUIDITY SWEEP (confirmation signal)
4. HARD KILLZONE FILTER (only trade high-liquidity periods)

This is the COMPLETE ICT Silver Bullet as originally designed.
"""
...
print("🚀 SILVER BULLET PREMIUM - FINAL CORRECT IMPLEMENTATION")
print("=" * 70)
print("✅ ALL COMPONENTS + HARD KILLZONE FILTER")
...
# Run FINAL CORRECT strategy
print(f"\n🎯 Step 3: Running Detection (MSS + FVG + SWEEP + KILLZONE)...")
...
print(f"   Killzones: London AM (3-4am), NY AM (10-11am), NY PM (2-3pm)")
print(f"   ✅ HARD KILLZONE FILTER ENABLED")
...
for i in tqdm(range(lookback, len(bars_list) - lookback)):
    ...
    # Detect FVG and check for MSS confluence + SWEEP + KILLZONE
    if i >= 2:
                    ...
                    for mss in mss_events:
                        if mss['direction'] == 'bearish':
                            ...
                            if bar_diff <= config['max_bar_distance']:
                                ...
                                # CRITICAL: HARD KILLZONE FILTER
                                if not killzone_aligned:
                                    continue  # Skip trades outside killzones
                                ...
                                if quality_score <= config['max_quality_score']:
                                    ...
                                    if swept:
                                        ...
                                        # Verify entry is ALSO in killzone
                                        entry_kz_aligned, entry_kz_window = is_within_trading_hours(
                                            entry_bar['timestamp'], DEFAULT_TRADING_WINDOWS
                                        )

                                        if not entry_kz_aligned:
                                            continue  # Skip if entry outside killzone
                                        ...
                                        if swing_price < entry_price:
                                            ...
                                            detected_setups.append({
                                                ...
                                                'killzone_window': entry_kz_window,
                                                'volume_ratio': mss['volume_ratio'],
                                                'fvg_size': gap_size_dollars,
                                                'bar_diff': bar_diff,
                                                'sweep_detected': True,
...
print(f"\n   Detected {len(detected_setups)} setups WITH ALL FILTERS")
...
print(f"\n📊 FINAL CORRECT - BACKTEST RESULTS:")
print("=" * 70)
...
print(f"   Trades/Day: {daily_counts.mean():.2f} (range: {daily_counts.min()} - {daily_counts.max()})")
...
print(f"\nKillzone Breakdown:")
for kz in trades_df['killzone_window'].unique():
    if pd.notna(kz):
        kz_trades = trades_df[trades_df['killzone_window'] == kz]
        kz_win_rate = (kz_trades['pnl'] > 0).sum() / len(kz_trades) * 100
        kz_pnl = kz_trades['pnl'].sum()
        print(f"   {kz:15s}: {len(kz_trades):2d} trades, {kz_win_rate:5.1f}% win, ${kz_pnl:7,.0f}")
...
for direction in ['bullish', 'bearish']:
    ...
    if len(subset) > 0:
        ...
        pnl = subset['pnl'].sum()
        print(f"   {direction.capitalize():10s}: {win_rate:5.1f}% win, ${pnl:7,.0f} ({len(subset)} trades)")
...
for reason in ['stop', 'target', 'timeout']:
    ...
    if len(reason_trades) > 0:
        ...
        print(f"   {reason.capitalize():10s}: {len(reason_trades):2d} trades ({len(reason_trades)/len(trades_df)*100:.1f}%) - {reason_win_rate:.1f}% win rate")

# Annual projection
trading_days = 252
avg_trades_per_day = len(trades_df) / len(daily_counts)
annual_trades = avg_trades_per_day * trading_days
annual_return = total_return * (trading_days / len(daily_counts))

print(f"\n📈 ANNUAL PROJECTION:")
print(f"   Projected Annual Trades: {annual_trades:.0f}")
print(f"   Projected Annual Return: ${annual_return:,.0f}")
print(f"   Projected Daily Return: ${annual_return/trading_days:,.0f}")

print(f"\n{'=' * 70}")
print("📊 PROGRESSION: FIXING THE STRATEGY")
print("=" * 70)
print(f"{'Version':<25s} {'Trades':>10s} {'Win Rate':>12s} {'Total P&L':>15s}")
print("-"*70)
print(f"{'Original (5-min)':<25s} {15:>10s} {60.00:>12.2f}% {20000:>15,.0f}")
print(f"{'1-min (no sweeps)':<25s} {146:>10s} {20.55:>12.2f}% {-83672:>15,.0f}")
print(f"{'+ Liquidity Sweeps':<25s} {65:>10s} {44.62:>12.2f}% {-19064:>15,.0f}")
print(f"{'+ Hard Killzone Filter':<25s} {len(trades_df):>10s} {win_rate:>12.2f}% {total_return:>15,.0f}")

print(f"\n{'='*70}")
if win_rate > 40 and profit_factor > 1.0:
    print("✅ STRATEGY FIXED - PROFITABLE WITH ALL CORRECT COMPONENTS")
    print("   Ready for deployment to paper trading")
elif win_rate > 35 and profit_factor > 0.8:
    print("⚠️  GETTING CLOSE - Need minor adjustments")
    print("   Consider wider stops or bearish-only filter")
else:
    print("❌ STILL NEEDS WORK - Killzones not the answer")
    print("   May need different approach")
print("="*70)

# Save results
output_path = Path("data/reports/premium_backtest_final_correct.csv")
