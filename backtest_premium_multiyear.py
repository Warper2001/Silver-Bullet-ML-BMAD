#!/usr/bin/env python3
"""
Multi-Year Backtest for Premium Strategy - ML Training Data Generation

Runs optimized premium strategy on 2023-2025 data to generate
sufficient sample size for ML model training.
"""
...
print("🚀 SILVER BULLET PREMIUM - MULTI-YEAR BACKTEST")
print("=" * 70)
print("Period: 2023-2025 (ML training data generation)")
print("=" * 70)

# Load multi-year OHLCV data
print(f"\n📊 Step 1: Loading multi-year MNQ data...")

import h5py
data_dir = Path("data/processed/time_bars/")
h5_files = sorted(data_dir.glob("MNQ_time_bars_5min_202*.h5"))
...
# Run multi-year premium strategy detection
print(f"\n🎯 Step 2: Running Premium Pattern Detection (MULTI-YEAR)...")
...
print(f"   Period: 2023-2025 (multi-year)")
...
print(f"   Goal: Generate 500+ trades for ML training")
...
print(f"\n   Detected {len(detected_setups)} multi-year setups")
...
for year, count in yearly_counts.items():
    ...
    year_win_rate = (year_trades['pnl'] > 0).sum() / len(year_trades) * 100
    year_return = year_trades['pnl'].sum()
    print(f"   {year}: {count:4d} trades, {year_win_rate:5.1f}% win, ${year_return:10,.0f}")
...
# Save results
output_path = Path("data/reports/premium_backtest_multiyear.csv")
output_path.parent.mkdir(parents=True, exist_ok=True)
trades_df.to_csv(output_path, index=False)

summary_path = Path("data/reports/premium_backtest_multiyear_summary.json")
with open(summary_path, 'w') as f:
    json.dump({
        ...
        'ml_training_ready': len(trades_df) >= 500,
        'sample_size': len(trades_df),
        'yearly_breakdown': {str(int(y)): int(c) for y, c in yearly_counts.items()}
...
if len(trades_df) >= 500:
    ...
    print(f"   Sufficient sample size: {len(trades_df)} trades")
    print(f"   Next: Run generate_ml_training_data.py with premium parameters")
else:
    print(f"⚠️  SAMPLE SIZE STILL INSUFFICIENT: {len(trades_df)} trades")
    print(f"   Consider: Even wider quality range or longer time period")
