#!/usr/bin/env python3
"""
Optimized Backtest for Premium Strategy with Diagnostic Fixes

This script implements the critical fixes identified from diagnostic analysis:
1. Invert FVG size filter (smaller FVGs work better)
2. Fix quality scoring (lower quality = better performance)
3. Wider stop losses (reduce premature stop hits)
"""
...
print("🚀 SILVER BULLET PREMIUM - OPTIMIZED BACKTEST")
print("=" * 70)
print("Method: Diagnostic-based optimization")
...
# Run premium strategy detection with OPTIMIZED parameters
print(f"\n🎯 Step 2: Running Premium Pattern Detection (OPTIMIZED)...")

# OPTIMIZED Configuration based on diagnostic analysis
config = {
    'min_fvg_gap': 75.0,           # Keep original minimum
    'max_fvg_gap': 500.0,          # NEW: Maximum to avoid huge FVGs (was too strict at 300)
    'min_volume_ratio': 2.0,
    'max_bar_distance': 7,
    'min_quality_score': 60.0,     # INVERTED: Lower minimum (was 70)
    'max_quality_score': 80.0,     # NEW: Maximum (was too strict at 75)
    'stop_multiplier': 1.5,        # MODERATE: Wider stops but not too aggressive (was 2.5)
    'killzone_weights': {
        'London AM': 0.90,
        'NY PM': 0.80,
        'NY AM': 0.60,
    ...
    'dow_multipliers': {
        ...
        'Tuesday': 0.6,
        'Wednesday': 1.0,
        'Thursday': 1.1,           # Best performing day
        'Friday': 1.0,
    },
...
print(f"   FVG Size: ${config['min_fvg_gap']}-${config['max_fvg_gap']} (avoid huge FVGs)")
print(f"   Quality Score: {config['min_quality_score']}-{config['max_quality_score']} (INVERTED!)")
print(f"   Stop Multiplier: {config['stop_multiplier']}x (moderately wider stops)")
print(f"   Focus Day: Thursday (best performance)")
...
for i in range(lookback, len(bars_list) - lookback):
    ...
    # Detect FVG with OPTIMIZED size filter
    if i >= 2:
        ...
        if candle_1['close'] > candle_3['open']:
            ...
            if top > bottom:
                ...
                # OPTIMIZED: Check both min and max FVG size
                if config['min_fvg_gap'] <= gap_size_dollars <= config['max_fvg_gap']:
                    ...
                    for mss in mss_events:
                        if mss['direction'] == 'bearish':
                            ...
                            if bar_diff <= config['max_bar_distance']:
                                ...
                                # OPTIMIZED: Invert quality threshold
                                # Only take lower quality setups (60-75 range)
                                if config['min_quality_score'] <= quality_score <= config['max_quality_score']:
                                    ...
                                    if swing_price < entry_price:  # Support must be below entry
                                        # OPTIMIZED: Apply wider stop multiplier
                                        base_stop = swing_price
                                        stop_distance = abs(entry_price - base_stop) * config['stop_multiplier']
                                        optimized_stop = entry_price - stop_distance  # Further below entry
                                        ...
                                        detected_setups.append({
                                            ...
                                            'entry': entry_price,
                                            'stop': optimized_stop,
                                            'target': entry_price + (entry_price - optimized_stop) * 2,  # 2:1 R/R
                            ...
                            if bar_diff <= config['max_bar_distance']:
                                ...
                                if config['min_quality_score'] <= quality_score <= config['max_quality_score']:
                                    ...
                                    if swing_price > entry_price:  # Resistance must be above entry
                                        ...
                                        optimized_stop = entry_price + stop_distance  # Further above entry
                                        ...
                                        detected_setups.append({
                                            ...
                                            'entry': entry_price,
                                            'stop': optimized_stop,
                                            'target': entry_price - (optimized_stop - entry_price) * 2,  # 2:1 R/R
                                            'quality_score': quality_score,
                                            'killzone_window': kz_window,
                                            'volume_ratio': mss['volume_ratio'],
                                            'fvg_size': gap_size_dollars,
...
print(f"\n   Detected {len(detected_setups)} optimized premium setups")
...
print("=" * 70)
print(f"{'Metric':<20s} {'Before':>15s} {'After':>15s} {'Change':>15s}")
print("-" * 70)
...
print(f"{'Trades/Day':<20s} {'9.8':>15s} {f'{daily_counts.mean():.1f}':>15s} {f'{daily_counts.mean() - 9.8:+.1f}':>15s}")

# Success assessment
print(f"\n{'=' * 70}")
if win_rate > 50 and profit_factor > 1.0 and total_return > 0:
    print("✅ OPTIMIZATION SUCCESSFUL - Strategy is now profitable!")
    print("   Ready for ML training data generation")
elif win_rate > 40 and profit_factor > 0.8:
    print("⚠️  OPTIMIZATION IMPROVED - Strategy shows promise")
    print("   Further parameter tuning recommended")
else:
    print("❌ OPTIMIZATION INSUFFICIENT - Strategy needs more work")
    print("   Consider alternative approaches")

print("=" * 70)

# Save results
output_path = Path("data/reports/premium_backtest_optimized.csv")
output_path.parent.mkdir(parents=True, exist_ok=True)
trades_df.to_csv(output_path, index=False)

summary_path = Path("data/reports/premium_backtest_optimized_summary.json")
with open(summary_path, 'w') as f:
    json.dump({
        ...
        'optimization_params': {
            'fvg_size_range': f"${config['min_fvg_gap']}-${config['max_fvg_gap']}",
...
print("=" * 70)
print("Based on diagnostic analysis, the following optimizations were applied:")
print("1. Inverted FVG size filter (smaller FVGs = better)")
print("2. Inverted quality score (lower quality = better)")
print("3. Wider stop losses (2.5x to reduce premature stops)")
