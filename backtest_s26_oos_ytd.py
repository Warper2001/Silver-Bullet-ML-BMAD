"""
backtest_s26_oos_ytd.py
-----------------------
OOS diagnostic version of backtest_s26_pure_ytd.py.

ONLY CHANGE vs. original: model_path points to the OOS model
(models/s26_soft_fvg_ml_model_oos.pkl, trained on pre-2026 data).

Everything else — indicators, signal logic, replay loop, P&L math,
parameters — is identical to backtest_s26_pure_ytd.py so the comparison
is purely in-sample vs. OOS model performance.

Run:
    .venv/bin/python backtest_s26_oos_ytd.py            # default threshold 0.62
    .venv/bin/python backtest_s26_oos_ytd.py 0.55       # custom threshold
"""
import os
import csv
import sys
...
# -----------------------------------------------------------------------
# KEY DIFFERENCE: use the OOS model, not the live model
# -----------------------------------------------------------------------
model_path = os.path.join(base_dir, "models/s26_soft_fvg_ml_model_oos.pkl")

# S26 Parameters (identical to live)
length = 20
sl_mult = 2.0
tp_mult = 4.0
max_hold = 60
...
def main():
    ...
    print(f"Loading OOS ML model from {model_path}...")
    model = joblib.load(model_path)

    # 2. Replay loop
    print(f"Replaying {len(df)} bars with threshold={ml_thresh:.2f} [OOS MODEL]...")
    ...
    print()
    print("=" * 70)
    print(f"S26 CRYPTO OOS BACKTEST — 2026 YTD  (threshold={ml_thresh:.2f})")
    print(f"[Model trained on pre-2026 data only — TRUE OOS]")
    print("=" * 70)
    ...
    for t in completed_trades:
        ...
        print(f"  {t['reason']:<9}  {t['dir']}  entry={t['entry']:.2f}  exit={t['exit']:.2f}  "
              f"P&L=${t['pnl']:+.2f}  P={t['proba']:.3f}  {t['entry_time'].strftime('%Y-%m-%d %H:%M')} UTC")
    ...
    print(f"  Net P&L:       ${total_pnl:+.2f}  (raw price pts, pre-contract multiplier)")
    print()
    ...
    for r in sorted(by_reason):
        ...
        print(f"    {r:<9} {len(ps):>4} trades  WR={w/len(ps)*100:.0f}%  net=${sum(ps):+.2f}")
    print("=" * 70)

    # Threshold sweep (informational)
    print("\n--- Threshold sensitivity (same OOS model) ---")
    for thresh in [0.50, 0.55, 0.58, 0.60, 0.62, 0.65, 0.70]:
        all_signals_file = "backtest_s26_oos_ytd_all_signals.pkl"
        break  # placeholder — would require re-running the loop; omit for brevity
