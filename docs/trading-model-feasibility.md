...
The audit takes repeated `--input` CSV arguments, `--output-dir`, and `--source-revision` (a full Git SHA). Inputs must match the fixed registered development-data allowlist; arbitrary CSVs and live ledgers are refused before content is read. The data root is this checkout, or its main checkout when running in `.claude/worktrees/`. Only synthetic tests replace the allowlist; there is no command-line bypass.

It writes `report.json`, `report.md` and a final `COMPLETE.json` containing both artifact hashes. A usable report requires a valid completion marker and matching hashes. An interrupted publication may leave an incomplete directory: retain it as diagnostic evidence and rerun to a new destination, never treat it as complete. Exit zero means the descriptive audit completed, **not** that data or power passed; always inspect the explicit gate fields. Missing and malformed datasets are reported. Unsafe paths, changed inputs, preregistration mismatches and existing destinations refuse the run.
...
The two grid hypotheses correspond to 09:30–15:59 or 09:31–16:00 America/New_York minute labels. Counts assume an ordinary weekday session, exclude duplicate contract-days, and are descriptive only. Parseable identities are counted for duplicates even when their prices are invalid. Invalid row reasons and up to twelve end-line examples are recorded, without copying input values into diagnostics. Missing contract labels produce an unassessable mixed-contract count, not a claim of zero mixing. Holidays, early closes, entirely missing sessions, bar completion, revisions and arrival times require independent evidence. Multiple contracts and overlapping files cannot be summed into independent sessions. No roll choice, interpolation, price adjustment or favourable-session selection occurs.
