...
def get_report():
    ...
    for trader_id in traders:
        ...
        # Map DB columns to required PerformanceMetricsCalculator columns
        # Calculator needs: timestamp, entry_price, exit_price, direction, pnl, exit_reason
        # AND for duration: entry_time, exit_time
        df['pnl'] = pd.to_numeric(df['pnl'], errors='coerce').fillna(0.0)
        df['timestamp'] = pd.to_datetime(df['timestamp'], utc=True)
        
        # Mapping: Assume timestamp is the exit time for now, as that's what we log.
        # Without explicit entry_time, duration will be 0.
        df['exit_time'] = df['timestamp']
        df['entry_time'] = df['timestamp'] # Placeholder to prevent errors
