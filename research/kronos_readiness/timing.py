"""Offline pinned CPU timing on bundled synthetic context; never price inputs."""
...
from research.kronos_replay.providers import CachedProvider
...
def run(
    ...
    decisions: int = 3,
    *,
    factory: Callable = CachedProvider,
    ...
    # Aggregate only fixture minutes. No replay accounting or real prices are used.
    aggregates = []
    for offset in range(0, len(bars), 15):
        chunk = bars[offset : offset + 15]
        row = {
        ...
        row["amount"] = row["volume"] * sum(row[k] for k in pilot.PRICE_COLUMNS) / 4
    ...
    try:
        ...
        for index in range(decisions):
            ...
            decision_checks = []
            for seed, path in zip(pilot.SEEDS, paths):
                decision_checks.append(pilot.validate_forecast(path, future))
            valid = all(c["invalid_candles"] == 0 for c in decision_checks)
            terminal = (
                statistics.mean(float(path.close.iloc[-1]) for path in paths)
                if valid
                else None
            )
            target = (
                (
                    int(terminal > context.close.iloc[-1])
                    - int(terminal < context.close.iloc[-1])
                )
                if valid
                else 0
            ...
            for seed, path in zip(pilot.SEEDS, paths):
                path.to_csv(
                    output / f"decision-{index}-seed-{seed}.csv",
                    index_label="timestamp",
                )
                (output / f"decision-{index}-seed-{seed}.csv").chmod(0o444)
        status = (
            "TIMING_COMPLETED"
    ...
    return finish(
        output,
        {
            **FLAGS,
            ...
            "context_sha256": sha((output / "synthetic-context.csv").read_bytes()),
            ...
            "interpretation": "Repeated fixed synthetic context, three sequential seeds per decision; no economic outcomes.",
        },
    )
