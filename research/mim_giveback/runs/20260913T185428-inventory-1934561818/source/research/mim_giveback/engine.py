"""Frozen arm-A execution model with one optional giveback exit overlay."""
...
from research.mim_robustness.artifacts import CONFIG
...
def simulate(
    ...
    quantity=1,
    feature_values=None,
    giveback_threshold=None,
    external_trigger_minutes=None,
):
    values = (
    ...
    def event(i, delta, fill, ref, reason):
        nonlocal p, entry, anchor, reference, gross, turnover, deactivated, ready, reset_bar, mfe
        old = p
    ...
    for i, row in enumerate(bars.itertuples(index=False)):
        ...
        if p:
            favorable = max(
                p * (float(row.high) - entry) * 2 * quantity,
                p * (float(row.low) - entry) * 2 * quantity,
            )
        ...
        current_gross = p * (c - entry) * 2 * quantity if p else 0.0
        giveback_ratio = (mfe - current_gross) / mfe if p and mfe > 0 else None
        external_trigger = bool(
            p
            and external_trigger_minutes is not None
            and minute in external_trigger_minutes
        )
        aligned_trigger = bool(
            p
            and giveback_threshold is not None
            and giveback_ratio is not None
            and giveback_ratio >= giveback_threshold
        )
        if target == p and (external_trigger or aligned_trigger):
            target = 0
        ...
        decisions.append(
            {
                ...
                "target": target,
                "mfe": mfe if p else None,
                "current_gross": current_gross if p else None,
                "giveback_ratio": giveback_ratio,
                "giveback_trigger": bool(reason == "GIVEBACK_EXIT"),
                "reference_realized_gross": reference,
                "reason": reason,
                "eligible": True,
                "exclusion": None,
