"""
S17 Gap Filter Calibration — Program C Phase 2a (Round 3)

Calibrates MIN_GAP_ATR_RATIO across 5 levels (0.15→0.35) on the
winning H4·M15-entry base architecture from Rounds 1 and 2.

Architecture fixed: H4 sweep + M15 entry (confirmed dominant in R1/R2).
Vary: gap_ratio ∈ {0.15, 0.20, 0.25, 0.30, 0.35} × confirm ∈ {none, M15, M5}.

Output: dose-response tables + IR score (PF×√N) + pre-registration candidate.

Usage:
    .venv/bin/python s17_gap_filter_calibration.py
"""
...
# ── Frozen parameters (identical to Phase 1 / S14–S16) ───────────────────────
SL_MULT           = 5.0
TP_MULT           = 6.0
ATR_PERIOD        = 20
VOL_LOOKBACK      = 120
...
# Fixed for all R3 combinations
SWEEP_TF_HOURS  = 4    # H4 sweep
ENTRY_TF_MIN    = 15   # M15 entry
MAX_HOLD_MIN    = 180  # 12 bars × 15 min
MAX_PENDING_MIN = 240  # 16 bars × 15 min

# Bonferroni for 12 combinations at alpha=0.05
BONFERRONI_T_CRIT = 2.69
MIN_N_FOR_CANDIDATE = 30   # minimum trades to be considered pre-reg candidate
...
CascadeConfig = namedtuple("CascadeConfig", [
    "sweep_tf_hours",  # fixed: 4
    "confirm_tf_min",  # 0=none, 5=M5, 15=M15
    "entry_tf_min",    # fixed: 15
    "gap_ratio",       # MIN_GAP_ATR_RATIO: 0.15 | 0.20 | 0.25 | 0.30 | 0.35
    "name",
])

GAP_LEVELS = [0.15, 0.20, 0.25, 0.30, 0.35]

COMBINATIONS = [
    # H4·NoConf·M15 — gap ratio sweep
    CascadeConfig(4, 0,  15, 0.15, "H4·NoConf·M15·g0.15"),  # R1/R2 ref
    CascadeConfig(4, 0,  15, 0.20, "H4·NoConf·M15·g0.20"),
    CascadeConfig(4, 0,  15, 0.25, "H4·NoConf·M15·g0.25"),  # R2 tight ref
    CascadeConfig(4, 0,  15, 0.30, "H4·NoConf·M15·g0.30"),
    CascadeConfig(4, 0,  15, 0.35, "H4·NoConf·M15·g0.35"),
    # H4·M15·M15 — gap ratio sweep
    CascadeConfig(4, 15, 15, 0.15, "H4·M15·M15·g0.15"),     # R1/R2 ref
    CascadeConfig(4, 15, 15, 0.20, "H4·M15·M15·g0.20"),
    CascadeConfig(4, 15, 15, 0.25, "H4·M15·M15·g0.25"),
    CascadeConfig(4, 15, 15, 0.30, "H4·M15·M15·g0.30"),
    CascadeConfig(4, 15, 15, 0.35, "H4·M15·M15·g0.35"),
    # H4·M5·M15 cross-check at top two ratios
    CascadeConfig(4, 5,  15, 0.25, "H4·M5·M15·g0.25"),
    CascadeConfig(4, 5,  15, 0.30, "H4·M5·M15·g0.30"),
]
...
class PrecomputedTFs:
    def __init__(self, bars_1min: list):
        ...
        print(f"  M5: {len(self.bars_m5):,}  M15: {len(self.bars_m15):,}  "
              f"H1: {len(self.bars_h1):,}  H4: {len(self.bars_h4):,}")
...
def run_cascade(bars_1min: list, precomp: PrecomputedTFs, cfg: CascadeConfig):
    """Run one H4·M15-entry cascade with the specified gap ratio. Returns (pnl, n_trades)."""
    ...
    for i, bar in enumerate(bars_1min):
        ...
        # Advance active trade (1-min resolution for realistic exits)
        if active:
        ...
        # Detect new H4 sweep
        if not sweep_active:
            cur_htf_idx = precomp.h4_idx[i]
            if cur_htf_idx >= 4 and cur_htf_idx > last_htf_idx:
                last_htf_idx = cur_htf_idx
                detected, sweep_ts = detect_bearish_sweep(precomp.bars_h4, cur_htf_idx)
                if detected:
                    sweep_active     = True
...
def build_report(results: list, bars_1min: list) -> str:
    ...
    W = 80
    lines.append("=" * W)
    lines.append("S17 Gap Filter Calibration — Program C Phase 2a (Round 3)")
    lines.append(f"Data: {DATA_CSV}")
    lines.append(f"Bars: {len(bars_1min):,}  "
                 f"({bars_1min[0].timestamp.date()} → {bars_1min[-1].timestamp.date()})")
    lines.append(f"Architecture: H4 sweep + M15 entry (fixed)  |  12 combinations")
    ...
    lines.append("")
    lines.append(f"DSR * = t > {BONFERRONI_T_CRIT} (Bonferroni alpha/12)  "
    ...
    for group_label, conf_min in groups:
        # Filter by confirm_tf_min; group_label prefix used only for display
        group = [(c, s) for c, s in results if c.confirm_tf_min == conf_min]
        if not group:
            continue
        lines.append(f"  {group_label}:")
        lines.append(f"  {'gap_ratio':>9}  {'PF':>6}  {'N':>4}  {'IR':>5}  {'trend':>6}")
        prev_pf = None
    ...
    if eligible:
        ...
        lines.append(f"    N    = {best_stats['n']}")
        lines.append(f"    IR   = {best_stats['ir']:.2f}  (highest PF×√N with N≥{MIN_N_FOR_CANDIDATE})")
        ...
        lines.append("")
        lines.append("  Runner-up candidates (N ≥ 30, sorted by IR):")
        for cfg2, s2 in sorted(eligible, key=lambda x: x[1]['ir'], reverse=True)[1:4]:
            lines.append(f"    {cfg2.name:<26}  PF={s2['pf']:.4f}  N={s2['n']}  IR={s2['ir']:.2f}")
    else:
        lines.append("  No eligible candidates (all combos have N < 30).")
...
def main():
    ...
    for k, cfg in enumerate(COMBINATIONS, 1):
        print(f"[{k:2d}/{n_total}] {cfg.name:<26}", end="", flush=True)
    ...
    out_path = REPORTS_DIR / f"s17_gap_calibration_{stamp}.txt"
    out_path.write_text(report)
    print(f"\nReport written → {out_path}")
    print("Round 3 complete. Pre-registration candidate identified — see report.")
