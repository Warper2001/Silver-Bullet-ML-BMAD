"""
S18 H4·M15·M15·g0.25 Holdout Test — Program C Phase 2a
Pre-registration: _bmad-output/preregistration_h4_m15_m15_g025.md

Architecture: H4 liquidity sweep → M15 CHoCH confirmation → M15 FVG entry
Gap filter: MIN_GAP_ATR_RATIO = 0.25 (tightened from Phase 1's 0.15)

Decision rule (pre-committed):
  N < 10                          → insufficient_sample
  N ≥ 10 AND PF ≤ 1.1350         → no_edge
  N ≥ 10 AND PF > 1.1350 ≤ 1.6221 → edge_confirmed
  N ≥ 10 AND PF > 1.6221         → edge_exceeds_insample

Usage:
    .venv/bin/python s18_h4_m15_m15_holdout.py --preregistration <SHA>
"""
...
# ── Frozen parameters (pre-registration: preregistration_h4_m15_m15_g025.md) ─
SL_MULT           = 5.0
TP_MULT           = 6.0
ATR_PERIOD        = 20
VOL_LOOKBACK      = 120
VOL_THRESH        = 0.75
ATR_THRESHOLD     = 0.5
MIN_GAP_ATR_RATIO = 0.25      # ← changed from Phase 1's 0.15
...
# S18 decision thresholds (pre-committed)
...
# ── Access gate (verbatim pattern from s12/s14) ───────────────────────────────
...
def append_access_log(sha: str, argv: list) -> None:
    ...
    log_row  = f"| {date_str} | {sha} | s18 script | `{argv_str}` | pending |\n"
...
def run_strategy(bars_1min: list):
    """H4·M15·M15·g0.25 cascade. Returns (pnl, n_trades)."""
...
def build_report(pf, n_trades, pnl, verdict, sha: str) -> str:
    lines = []
    lines.append("=" * 70)
    lines.append("S18 H4·M15·M15·g0.25 Holdout Test — Program C Phase 2a")
    lines.append(f"Pre-registration SHA: {sha}")
    lines.append(f"Pre-registration doc: _bmad-output/preregistration_h4_m15_m15_g025.md")
    ...
    lines.append(f"  MIN_GAP_ATR_RATIO = {MIN_GAP_ATR_RATIO}  (Phase 1 was 0.15)")
    lines.append(f"  MAX_HOLD = {MAX_HOLD_MIN} min  MAX_PENDING = {MAX_PENDING_MIN} min")
    lines.append("")

    lines.append("── S18 Results ──────────────────────────────────────────────────────")
    ...
    lines.append(f"  S18 in-sample PF (2025) : {INSAMPLE_PF:.4f}  (N=41)")
    lines.append(f"  Minimum N for verdict   : {MIN_N}")
    lines.append("")

    lines.append("── S18 Verdict (pre-committed decision rule) ────────────────────────")
    if verdict == "insufficient_sample":
        lines.append(f"  Verdict: {verdict}")
        lines.append(f"  N={n_trades} < {MIN_N} minimum. Result is directional signal only.")
        lines.append("  Do NOT make architectural decisions based on this result alone.")
        lines.append("  Consider pre-registering H4·M15·M15·g0.20 (N=49 in-sample) for")
        lines.append("  a higher-frequency holdout test before concluding.")
    elif verdict == "no_edge":
        lines.append(f"  Verdict: {verdict}")
        lines.append(f"  PF {pf_str} ≤ S12 p90 random {S12_P90_RANDOM:.4f}.")
        lines.append("  H4·M15·M15 has no edge at g0.25 on the holdout.")
        lines.append("  Evaluate g0.20 pre-registration or PIVOT.")
        ...
        lines.append("  Architecture: H4·M15·M15·g0.25 is the new baseline.")
...
def main():
    parser = argparse.ArgumentParser(description="S18 H4·M15·M15·g0.25 Holdout Test")
    parser.add_argument("--preregistration", metavar="SHA",
                        help="Git SHA of the S18 pre-registration commit (required)")
    ...
    if not args.preregistration:
        ...
        print(
            ...
            "Commit _bmad-output/preregistration_h4_m15_m15_g025.md first,\n"
    ...
    print("Running H4·M15·M15·g0.25 strategy …\n")
    ...
    out_path = REPORTS_DIR / f"s18_{stamp}.txt"
