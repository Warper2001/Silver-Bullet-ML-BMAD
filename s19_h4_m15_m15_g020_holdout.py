"""
S19 H4·M15·M15·g0.20 Holdout Test — Program C Phase 2a
Pre-registration: _bmad-output/preregistration_h4_m15_m15_g020.md

Architecture: H4 liquidity sweep → M15 CHoCH confirmation → M15 FVG entry
Gap filter: MIN_GAP_ATR_RATIO = 0.20 (prescribed secondary test after S18 insufficient_sample)

Decision rule (pre-committed):
  N < 10                          → insufficient_sample
  N ≥ 10 AND PF ≤ 1.1350         → no_edge
  N ≥ 10 AND PF > 1.1350 ≤ 1.3226 → edge_confirmed
  N ≥ 10 AND PF > 1.3226         → edge_exceeds_insample

Usage:
    .venv/bin/python s19_h4_m15_m15_g020_holdout.py --preregistration <SHA>
"""
...
# ── Frozen parameters (pre-registration: preregistration_h4_m15_m15_g020.md) ─
SL_MULT           = 5.0
TP_MULT           = 6.0
ATR_PERIOD        = 20
VOL_LOOKBACK      = 120
VOL_THRESH        = 0.75
ATR_THRESHOLD     = 0.5
MIN_GAP_ATR_RATIO = 0.20      # ← g0.20 (S18 was 0.25; prescribed secondary test)
...
ENTRY_TF_MIN    = 15   # M15 FVG
MAX_HOLD_MIN    = 180  # 12 M15 bars
MAX_PENDING_MIN = 240  # 16 M15 bars

# S19 decision thresholds (pre-committed)
S12_P90_RANDOM   = 1.1350
INSAMPLE_PF      = 1.3226   # H4·M15·M15·g0.20 in-sample PF (N=49, 2025)
MIN_N            = 10
...
# ── Access gate (verbatim pattern from s12/s14/s18) ──────────────────────────
...
def append_access_log(sha: str, argv: list) -> None:
    ...
    log_row  = f"| {date_str} | {sha} | s19 script | `{argv_str}` | pending |\n"
...
# ── Vol regime (H1 ATR percentile, same as all prior scripts) ────────────────
...
def run_strategy(bars_1min: list):
    """H4·M15·M15·g0.20 cascade. Returns (pnl, n_trades)."""
    bars_m15 = resample_bars(bars_1min, CONFIRM_TF_MIN)   # M15 = confirm and entry TF
    bars_h4  = resample_bars(bars_1min, SWEEP_TF_HOURS * 60)
    bars_h1  = resample_bars(bars_1min, 60)

    m15_idx = build_completed_idx(bars_1min, bars_m15, CONFIRM_TF_MIN)
    h4_idx  = build_completed_idx(bars_1min, bars_h4,  SWEEP_TF_HOURS * 60)
    ...
    for i, bar in enumerate(bars_1min):
        ...
        cur_h1 = h1_idx[i]
        if cur_h1 > last_h1_fed and cur_h1 >= 0:
            last_h1_fed = cur_h1
        ...
        if not sweep_active:
            cur_h4 = h4_idx[i]
            if cur_h4 >= 4 and cur_h4 > last_h4_idx:
                last_h4_idx = cur_h4
                detected, sweep_ts = detect_bearish_sweep(bars_h4, cur_h4)
                if detected:
                    sweep_active     = True
        ...
        # M15 FVG detection
        cur_m15 = m15_idx[i]
        if cur_m15 < 2 or cur_m15 <= last_m15_fvg:
            continue
        last_m15_fvg = cur_m15

        c1 = bars_m15[cur_m15 - 2]
        c2 = bars_m15[cur_m15 - 1]
        c3 = bars_m15[cur_m15]
...
def build_report(pf, n_trades, pnl, verdict, sha: str) -> str:
    lines = []
    lines.append("=" * 70)
    lines.append("S19 H4·M15·M15·g0.20 Holdout Test — Program C Phase 2a")
    lines.append(f"Pre-registration SHA: {sha}")
    lines.append(f"Pre-registration doc: _bmad-output/preregistration_h4_m15_m15_g020.md")
    lines.append(f"Holdout: {HOLDOUT_CSV}")
    lines.append("=" * 70)
    lines.append("")

    lines.append("── Architecture (frozen) ────────────────────────────────────────────")
    lines.append("  Sweep: H4 (4-hour) liquidity sweep, expires 24h after sweep bar")
    lines.append("  Confirm: M15 CHoCH (close < swing_low − 0.3×M15_ATR)")
    lines.append("  Entry: M15 bearish FVG, midpoint entry")
    lines.append(f"  MIN_GAP_ATR_RATIO = {MIN_GAP_ATR_RATIO}  (S18 was 0.25; Phase 1 was 0.15)")
    lines.append(f"  MAX_HOLD = {MAX_HOLD_MIN} min  MAX_PENDING = {MAX_PENDING_MIN} min")
    lines.append("")

    lines.append("── S19 Results ──────────────────────────────────────────────────────")
    ...
    lines.append(f"  S19 in-sample PF (2025)    : {INSAMPLE_PF:.4f}  (N=49, g0.20)")
    lines.append(f"  S18 result (g0.25, N=2)    : insufficient_sample")
    lines.append(f"  Minimum N for verdict      : {MIN_N}")
    lines.append("")

    lines.append("── S19 Verdict (pre-committed decision rule) ────────────────────────")
    if verdict == "insufficient_sample":
        lines.append(f"  Verdict: {verdict}")
        lines.append(f"  N={n_trades} < {MIN_N} minimum. Both g0.25 (S18) and g0.20 (S19) are too")
        lines.append("  sparse on this holdout window. This is an architectural concern.")
        lines.append("  Consider pivoting to a different architecture or extending the")
        lines.append("  holdout window in a new pre-registration.")
    elif verdict == "no_edge":
        lines.append(f"  Verdict: {verdict}")
        lines.append(f"  PF {pf_str} ≤ S12 p90 random {S12_P90_RANDOM:.4f}.")
        lines.append("  H4·M15·M15 has no edge at g0.20. Combined with S18 insufficient_sample,")
        lines.append("  this is a PIVOT signal. Review architectural options.")
        ...
        lines.append("  Architecture: H4·M15·M15·g0.20 is the new baseline.")
    else:  # edge_exceeds_insample
        lines.append(f"  Verdict: {verdict}")
        ...
        lines.append("  high confidence. Note: in-sample PF was the expected upper bound.")
...
def main():
    parser = argparse.ArgumentParser(description="S19 H4·M15·M15·g0.20 Holdout Test")
    parser.add_argument("--preregistration", metavar="SHA",
                        help="Git SHA of the S19 pre-registration commit (required)")
    ...
    if not args.preregistration:
        ...
        print(
            ...
            "Commit _bmad-output/preregistration_h4_m15_m15_g020.md first,\n"
    ...
    print("Running H4·M15·M15·g0.20 strategy …\n")
    ...
    # Update ACCESS_LOG with final verdict
    ...
    out_path = REPORTS_DIR / f"s19_{stamp}.txt"
