"""
S21 H1+M1 Gap Filter Calibration — Program C Phase 2b

S12 proved H1 sweep + M1 FVG entry has holdout edge (PF=1.2154, 96 trades).
S20 showed H1 sweep + M15 entry is weak (PF 0.98–1.14).
This screen tests whether tightening the gap filter improves the proven
H1+M1 architecture, and whether adding M15 CHoCH confirm helps.

Fixed: H1 sweep, M1 FVG entry.
Vary: gap_ratio ∈ {0.15 (baseline), 0.20, 0.25, 0.30}
      confirm   ∈ {none, M15 CHoCH}
→ 8 combinations.

Usage:
    .venv/bin/python s21_h1_m1_gap_filter.py
"""
...
# ── Frozen parameters (identical to Phase 1 / S12 baseline) ──────────────────
SL_MULT           = 5.0
TP_MULT           = 6.0
ATR_PERIOD        = 20
VOL_LOOKBACK      = 120
...
# Fixed for all S21 combinations
SWEEP_TF_HOURS  = 1    # H1 sweep (proven load-bearing in S14)
ENTRY_TF_MIN    = 1    # M1 FVG entry (Phase 1 / S12 baseline)
MAX_HOLD_MIN    = 60   # 60 M1 bars = 60 minutes
MAX_PENDING_MIN = 240  # 240 M1 bars = 240 minutes

# Bonferroni for 8 combinations at alpha=0.05
BONFERRONI_T_CRIT   = 2.66
MIN_N_FOR_CANDIDATE = 40
...
CascadeConfig = namedtuple("CascadeConfig", [
    ...
    "confirm_tf_min",  # 0=none, 15=M15 CHoCH
    "entry_tf_min",    # fixed: 1
    "gap_ratio",       # MIN_GAP_ATR_RATIO override
    "name",
])

COMBINATIONS = [
    # H1·NoConf·M1 — gap ratio sweep (S12 baseline is g0.15)
    CascadeConfig(1, 0,  1, 0.15, "H1·NoConf·M1·g0.15"),   # S12 baseline
    CascadeConfig(1, 0,  1, 0.20, "H1·NoConf·M1·g0.20"),
    CascadeConfig(1, 0,  1, 0.25, "H1·NoConf·M1·g0.25"),
    CascadeConfig(1, 0,  1, 0.30, "H1·NoConf·M1·g0.30"),
    # H1·M15CHoCH·M1 — same gap sweep with M15 confirm
    CascadeConfig(1, 15, 1, 0.15, "H1·M15·M1·g0.15"),
    CascadeConfig(1, 15, 1, 0.20, "H1·M15·M1·g0.20"),
    CascadeConfig(1, 15, 1, 0.25, "H1·M15·M1·g0.25"),
    CascadeConfig(1, 15, 1, 0.30, "H1·M15·M1·g0.30"),
]
...
def run_cascade(bars_1min: list, precomp: PrecomputedTFs, cfg: CascadeConfig):
    """Run H1·M1-entry cascade with specified gap ratio. Returns (pnl, n_trades)."""
    ...
    for i, bar in enumerate(bars_1min):
        ...
        # Advance active trade (bar-count based, consistent with Phase 1 / S12)
        if active:
            active_bars += 1
        ...
        # M1 FVG detection on current 1-min bar
        if i < 2:
            continue
...
def build_report(results: list, bars_1min: list) -> str:
    ...
    W = 80
    lines.append("=" * W)
    lines.append("S21 H1+M1 Gap Filter Calibration — Program C Phase 2b")
    lines.append(f"Data: {DATA_CSV}")
    lines.append(f"Bars: {len(bars_1min):,}  "
                 f"({bars_1min[0].timestamp.date()} → {bars_1min[-1].timestamp.date()})")
    lines.append(f"Architecture: H1 sweep + M1 entry (fixed)  |  8 combinations")
    lines.append(f"Baseline: H1·NoConf·M1·g0.15 = S12 pre-cutoff (PF≈1.03, N≈210)")
    ...
    lines.append("")
    lines.append(f"DSR * = t > {BONFERRONI_T_CRIT} (Bonferroni alpha/8)  "
...
def main():
    ...
    print("Pre-resampling to M15, H1...")
    ...
    out_path = REPORTS_DIR / f"s21_h1m1_gap_{stamp}.txt"
    out_path.write_text(report)
    print(f"\nReport written → {out_path}")
    print("S21 complete. Review pre-registration candidate above.")
