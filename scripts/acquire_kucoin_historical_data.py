#!/usr/bin/env python3
"""
KuCoin Historical Data Acquisition Script

This script fetches historical kline (candlestick) data from KuCoin API
...
Usage:
    python scripts/acquire_kucoin_historical_data.py --symbol BTC-USDT --interval 5m --days 365

API Documentation: https://docs.kucoin.com/#get-klines
"""
...
class KuCoinDataAcquirer:
    """
    Acquire historical kline data from KuCoin API.
            ...
            while current_start_time < end_time_ms:
                try:
                    ...
                    logger.debug(
                        ...
                        f"Fetching klines from {datetime.fromtimestamp(current_start_time / 1000, tz=timezone.utc)}"
                    )

                    # Fetch klines using KuCoin client
                    ...
                    current_start_time = klines[-1].close_time + 1

                    # Progress update
                    progress = min(
                        100,
                        (current_start_time - int(self.start_time.timestamp() * 1000))
                        / (end_time_ms - int(self.start_time.timestamp() * 1000)) * 100
                    )
                    logger.info(
                        f"Fetched {len(klines)} klines (total: {total_klines}, progress: {progress:.1f}%)"
                    )

                    # Rate limiting: KuCoin allows ~100 requests/second
    ...
    def validate_data(self, klines_data: list[list]) -> pd.DataFrame:
        ...
        """
        # Define column names (KuCoin API format)
        columns = [
            "open_time",
            "open",
            "high",
    ...
    def save_to_parquet(
        ...
        Args:
            ...
            output_dir: Output directory (default: data/kucoin/historical)
        ...
        if output_dir is None:
            output_dir = "data/kucoin/historical"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Acquire historical kline data from KuCoin"
    )
    parser.add_argument(
        "--symbol",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory (default: data/kucoin/historical)",
    )

    args = parser.parse_args()

    # Create acquirer
    acquirer = KuCoinDataAcquirer(
        symbol=args.symbol,
