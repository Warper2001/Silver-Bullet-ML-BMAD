#!/usr/bin/env python3
"""
Silver Bullet Premium - High Quality Low Frequency Variant

This variant implements stricter pattern confluence requirements:
✅ FVG depth filters (min $75 gap size)
✅ Pattern strength scoring (swing points, MSS volume ratio)
✅ Tighter confluence requirements (max 7 bars vs 20)
✅ Higher ML threshold (75% vs 65%)
✅ Daily trade limit (max 20 trades/day)
✅ Quality ranking system (min score 70/100)
"""
...
class PremiumConfig(BaseModel):
    """Configuration for Silver Bullet Premium strategy."""

    enabled: bool = True
    min_fvg_gap_size_dollars: float = 75.0
    mss_volume_ratio_min: float = 2.0
    max_bar_distance: int = 7
    ml_probability_threshold: float = 0.75
    require_killzone_alignment: bool = True
    ...
    @field_validator('max_bar_distance')
    @classmethod
    def validate_bar_distance(cls, v: int) -> int:
        if v < 1 or v > 50:
            raise ValueError('max_bar_distance must be between 1 and 50')
        return v

    @field_validator('ml_probability_threshold')
    @classmethod
    def validate_ml_threshold(cls, v: float) -> float:
        if v < 0.0 or v > 1.0:
            raise ValueError('ml_probability_threshold must be between 0 and 1')
        return v
...
def calculate_setup_quality_score(setup: Dict[str, Any]) -> float:
    ...
    Factors:
    - FVG size (larger = better)
    - MSS volume ratio (higher = better)
    - Bar alignment distance (closer = better)
    - Killzone alignment (aligned = better)
    - Swing point strength

    Args:
        setup: Dictionary containing setup details
    ...
    alignment_score = max(0, 100 - (bar_diff * 10))  # 0 bars = 100, 10+ bars = 0
...
class SilverBulletPremiumTrader:
    """Premium quality Silver Bullet strategy trader."""

    def __init__(self, access_token: str, config: PremiumConfig):
        """Initialize the premium strategy trader."""
        ...
        # Initialize ML components with premium model
        try:
            # Try to load premium model first
        ...
        logger.info("✅ Silver Bullet PREMIUM Strategy initialized")
        logger.info("📊 Premium Configuration:")
    ...
    def _reset_daily_trade_count(self) -> None:
        ...
        if self.last_trade_date is None or now.date() != self.last_trade_date.date():
            ...
            logger.info(f"📅 New trading day: {now.strftime('%Y-%m-%d')}")
    ...
    def detect_fvg_setups(self) -> List[Dict]:
        ...
        try:
            ...
            # Filter by minimum gap size (premium requirement)
            if bullish_fvg:
            ...
            for idx in range(recent_bar_start, len(self.recent_bars)):
                ...
                for swing_high in self.swing_highs[-5:]:
                    if current_bar.high > swing_high['price']:
                        ...
                        # Premium: Require higher volume ratio
        ...
        try:
            ...
            for fvg in self.fvg_setups:
                for mss in self.mss_events:
                    ...
                    # Time alignment (premium: tighter requirement)
                    ...
                    if quality_score >= self.config.min_quality_score:
                        setups.append(setup)
                        logger.info(f"🎯 Premium Setup: {fvg['direction'].upper()} (quality: {quality_score:.0f}/100)")
                    else:
                        logger.debug(f"Setup quality too low: {quality_score:.0f} < {self.config.min_quality_score}")
    ...
    async def process_trading_signal(self, setup: Dict) -> bool:
        ...
        # Filter 1: Quality score (already applied in detection, but log it)
        if setup['quality_score'] < self.config.min_quality_score:
            logger.info(f"❌ Rejected: Quality score {setup['quality_score']:.0f} < {self.config.min_quality_score}")
            self.quality_filtered_signals += 1
            return False

        # Filter 2: Killzone alignment
        if self.config.require_killzone_alignment and not setup['killzone_aligned']:
            logger.info(f"❌ Rejected: Not in killzone alignment")
        ...
        if self.daily_trade_count >= self.config.max_trades_per_day:
            logger.info(f"❌ Rejected: Max daily trades reached ({self.daily_trade_count}/{self.config.max_trades_per_day})")
        ...
        self.total_pnl += potential_profit * 0.8482

        logger.info(f"📈 Paper Profit: ${potential_profit:.2f} (1 contract)")
        logger.info(f"📊 Daily trades: {self.daily_trade_count}/{self.config.max_trades_per_day}")
        logger.info(f"✅ TRADE COMPLETE")

        return True

    async def run_trading_loop(self, symbol: str = "MNQM26"):
        """Main trading loop for premium strategy."""
        logger.info("🚀 Starting Silver Bullet PREMIUM Strategy")
        logger.info("=" * 80)
        ...
        logger.info(f"Max Daily Trades: {self.config.max_trades_per_day}")
        logger.info("=" * 80)
    ...
    async def stop(self):
        ...
        logger.info("🛑 Stopping Silver Bullet PREMIUM Strategy...")
        self.running = False
        ...
        logger.info("=" * 80)
        logger.info("✅ Silver Bullet PREMIUM Strategy stopped")
...
async def main():
    ...
    # Initialize trader
    trader = SilverBulletPremiumTrader(access_token, config)
