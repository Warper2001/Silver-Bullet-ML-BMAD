#!/usr/bin/env python3
"""TIER 1 FVG Paper Trading System (REST API).

Uses periodic REST API calls to fetch MNQ data instead of WebSocket.
More reliable for paper trading environment.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir / 'tier1_paper_rest.log'),
...
class Tier1PaperTradingREST:
    """TIER 1 FVG Paper Trading using REST API."""
    ...
    def __init__(self):
        ...
        logger.info("TIER 1 Paper Trading System (REST API) initialized")
        logger.info(f"Configuration: {TIER1_CONFIG}")

    async def initialize(self) -> bool:
        """Initialize authentication and HTTP client."""
        logger.info("="*60)
        logger.info("TIER 1 FVG PAPER TRADING (REST API)")
        logger.info("="*60)
        ...
        logger.info(f"Data Source: TradeStation REST API")
    ...
    async def load_initial_data(self):
        """Load initial historical bar data."""
        ...
        try:
            ...
            url = "https://api.tradestation.com/v3/marketdata/barcharts"
            ...
            if data.get("Bars"):
                logger.info(f"✓ Loaded {len(data['Bars'])} initial bars")
                ...
                for bar in data['Bars']:
                    dollar_bar = DollarBar(
                        ...
                        volume=int(bar['TotalVolume']),
                        notional_value=float(bar['Open']) * float(bar['TotalVolume']) * MNQ_POINT_VALUE,
                ...
                logger.warning("No initial bars loaded")
    ...
    async def fetch_new_bars(self):
        """Fetch new bar data from TradeStation."""
        try:
            access_token = await self.auth.authenticate()

            # Get recent bars
            if self.dollar_bars:
                last_time = self.dollar_bars[-1].timestamp
            else:
            ...
            if data.get("Bars"):
                new_bars = [bar for bar in data['Bars']
                           if datetime.fromisoformat(bar['Timestamp'].replace('Z', '+00:00')) > last_time]

                if new_bars:
                    logger.debug(f"Fetched {len(new_bars)} new bars")
