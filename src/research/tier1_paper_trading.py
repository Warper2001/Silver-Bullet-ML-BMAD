#!/usr/bin/env python3
"""TIER 1 FVG Real-Time Paper Trading System.

Implements the validated TIER 1 strategy with live TradeStation data.
...
class Tier1PaperTradingSystem:
    ...
    def __init__(self):
        ...
        self.ws_client: Optional[TradeStationWebSocketClient] = None
        ...
        logger.info("TIER 1 Paper Trading System initialized")
        logger.info(f"Configuration: {TIER1_CONFIG}")

    async def initialize(self) -> bool:
        """Initialize authentication and WebSocket connection."""
        logger.info("="*60)
        logger.info("TIER 1 FVG REAL-TIME PAPER TRADING")
        logger.info("="*60)
        ...
        try:
            ...
            self.ws_client = TradeStationWebSocketClient(self.auth)
            await self.ws_client.connect()

            logger.info("✓ WebSocket connected")
            logger.info("✓ Subscribing to MNQ market data...")

            # Subscribe to MNQ data
            self.data_queue = await self.ws_client.subscribe()

            logger.info("✓ Subscribed to MNQ market data")
    ...
    async def run(self):
        ...
        try:
            while self.running:
                ...
                if self.dollar_bars and len(self.dollar_bars) % 100 == 0:
                    self.log_status()
    ...
    async def process_market_data(self, market_data):
        """Process incoming market data and detect FVG setups."""
        # Add to raw data buffer
        self.raw_data_buffer.append({
            'timestamp': market_data.timestamp,
            'open': market_data.last or 0,  # Use last price as approximation
            'high': market_data.last or 0,
            'low': market_data.last or 0,
            'close': market_data.last or 0,
            'volume': market_data.volume or 0
        })

        # Check if we have enough data for dollar bar transformation
        if len(self.raw_data_buffer) < 100:  # Need minimum data
            return

        # Transform to dollar bars (simplified for real-time)
        await self.update_dollar_bars()
    ...
    async def update_dollar_bars(self):
        """Update dollar bars from raw data buffer."""
        # Simplified dollar bar creation for real-time
        # In production, use proper dollar bar transformation

        df = pd.DataFrame(self.raw_data_buffer)
        df['notional'] = ((df['high'] + df['low']) / 2) * df['volume'] * MNQ_POINT_VALUE
        df['cumulative_notional'] = df['notional'].cumsum()

        # Find dollar bar boundaries
        ...
        if len(bar_boundaries) > 0:
            ...
            latest_boundary = bar_boundaries[-1]

            if not self.dollar_bars or len(self.dollar_bars) == 0:
                start_idx = 0
            else:
                start_idx = max(0, latest_boundary - 100)  # Get recent data

            segment = df.iloc[start_idx:latest_boundary+1]

            if len(segment) > 0:
                dollar_bar = DollarBar(
                    timestamp=segment.iloc[-1]['timestamp'],
                    open=float(segment.iloc[0]['open']),
                    high=float(segment['high'].max()),
                    low=float(segment['low'].min()),
                    close=float(segment.iloc[-1]['close']),
                    volume=int(segment['volume'].sum()),
                    notional_value=float(segment['notional'].sum()),
                    is_forward_filled=False,
                )

                # Only add if it's a new bar
                if not self.dollar_bars or dollar_bar.timestamp > self.dollar_bars[-1].timestamp:
                    ...
                    if len(self.dollar_bars) > 1000:
                        self.dollar_bars = self.dollar_bars[-500:]
                        self.raw_data_buffer = self.raw_data_buffer[-1000:]
    ...
    async def detect_fvg_setups(self):
        ...
        i = len(self.dollar_bars) - 1  # Current bar
    ...
    async def shutdown(self):
        ...
        if self.ws_client:
            try:
                # WebSocket client doesn't have explicit close method
                pass
            except Exception as e:
