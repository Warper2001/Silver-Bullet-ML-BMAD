#!/usr/bin/env python3
"""TIER 1 FVG Paper Trading System (Working Version).

Uses the same proven patterns as existing working paper trading scripts.
...
# TradeStation API (working format)
...
class Tier1PaperTrader:
    """TIER 1 FVG Paper Trading - Working Implementation."""
    ...
    def __init__(self):
        ...
        logger.info("TIER 1 FVG Paper Trading System initialized")
        logger.info(f"Configuration: {TIER1_CONFIG}")

    async def initialize(self) -> bool:
        """Initialize system."""
        logger.info("="*60)
        logger.info("TIER 1 FVG PAPER TRADING")
        logger.info("="*60)
        ...
        if not token_file.exists():
            ...
            logger.info("Run OAuth flow first:")
            logger.info("  .venv/bin/python get_standard_auth_url.py")
            logger.info("  .venv/bin/python exchange_token_simple.py <code>")
            return False
    ...
    async def load_initial_bars(self):
        """Load initial bars using working API format."""
        ...
        try:
            ...
            # Use working API format (POST with JSON body)
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get("Bars"):
                    logger.info(f"✅ Loaded {len(data['Bars'])} bars")
                    ...
                    logger.info(f"✅ Created {len(self.dollar_bars)} dollar bars")
                    logger.info(f"✅ Indicators calculated")
                else:
                    logger.warning("No bars in response")
            else:
                logger.error(f"Failed to load bars: {response.status_code} - {response.text}")

        except Exception as e:
            logger.error(f"Failed to load initial bars: {e}")
            # Continue anyway, we'll fetch new data
    ...
    async def fetch_new_bars(self):
        """Fetch new bars using working API format."""
        try:
            current_token = await self.auth.authenticate()
            headers = {
                "Authorization": f"Bearer {current_token}",
            ...
            response = await self.http_client.post(
                ...
                json={
                    ...
                    "interval": BAR_INTERVAL,
                    "unit": BAR_UNIT,
                    "barscount": 10,  # Small batch for new data
                    "useEST": True,
                },
            )
                ...
                if data.get("Bars"):
                    ...
                    for bar_data in data["Bars"]:
                        bar = DollarBar(
                            ...
                            close=float(bar_data["Close"]),
                            volume=int(bar_data["TotalVolume"]),
                            notional_value=float(bar_data["Close"]) * 2,
                            is_forward_filled=False,
                        )

                        # Only add if newer than last bar
                        if not self.dollar_bars or bar.timestamp > self.dollar_bars[-1].timestamp:
                            self.dollar_bars.append(bar)
            ...
            logger.warning(f"Failed to fetch new bars: {e}")
