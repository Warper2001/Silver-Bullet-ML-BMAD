...
def main():
    ...
    optimized_hash = hashlib.sha256((ROOT/ADAPTER).read_bytes()).hexdigest()
    ...
    report = dict(scope='COMPONENT_DIAGNOSTIC_EXPLICIT_BUILTIN_UTC_NOT_POLL_LATENCY',
        ...
        optimized_adapter_sha256=optimized_hash,
    ...
    assert hashlib.sha256((ROOT/ADAPTER).read_bytes()).hexdigest() == optimized_hash, 'adapter changed during diagnostic'
