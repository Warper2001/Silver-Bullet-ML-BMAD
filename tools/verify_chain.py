...
`trades.csv` verifies cleanly right now while **missing the 2026-08-06 trade** and
**double-counting 2026-06-25**. It was reverted wholesale to an earlier commit, and an
earlier state of an append-only chain is itself a valid chain prefix. Completeness is a
separate question, answered by reconciling against `data/trades.db` (see --reconcile).

`decisions.csv` DOES fail — because the live process kept appending after the revert,
    ...
    for rel in (args.files or DEFAULT_FILES):
        ...
        if args.reconcile and rel in RECONCILE:
            ...
            if missing:
                ...
                worst = max(worst, BROKEN)
            if dupes:
                print(f"         DUPLICATED rows for: {dupes}")
                worst = max(worst, BROKEN)
