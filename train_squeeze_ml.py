...
# Instead of fillna on shift, fillna on the mask to avoid deprecation warnings
...
print(f"Training on {len(X_train)} trades...")
...
for thresh in [0.5, 0.52, 0.54, 0.56]:
    filtered = test_df[test_df['proba'] > thresh]
    calc_metrics(filtered, f"ML Filtered > {thresh}")

model_path = Path("/root/Silver-Bullet-ML-BMAD/models/vol_squeeze_ml_model.pkl")
